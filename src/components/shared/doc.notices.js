...
const DocNotices = (props) => {
    ...
    return <React.Fragment style={{ top: 10 }}>
        <div className="main-container">
            ...
            {props?.dashboard?.notices.loading === true ? <div className="text-center p-24"><Spin size="default" /></div> : <>{props?.dashboard?.notices?.data?.map(item => <Alert style={{ margin: 20, cursor: "pointer" }} type="error" showIcon onClick={() => props.history?.push("/documents?id=" + item.typeId)} message={item.title} />)}</>}
