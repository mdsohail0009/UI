...
class AddressCrypto extends Component {
  ...
  getCryptoData = async () => {
    ...
    if (response.ok) {
      this.setState({
        ...
        isLoading: false,
        isEdit: true,
        check: response.data.isOwnerOfWalletAddress,
        isDocCheck: response.data.isDocumentUpload,
        walletSourse: response.data?.walletSource
      });
    } else {
      this.setState({
        ...this.state,
  ...
  coinList = async () => {
    ...
    if (this.state.cryptoData.network) {
      ...
      this.form?.current?.setFieldsValue({
        network: this.state.cryptoData.network
      });
    } else if (this.props.sendReceive?.withdrawFiatObj?.walletCode) {
      this.form?.current?.setFieldsValue({
        token: this.props.sendReceive?.withdrawFiatObj?.walletCode
      });
  ...
  handleTokenChange = (value) => {
    ...
    if (value && walletAddress) {
      this.form?.current?.validateFields(
        ["walletAddress"],
        this.validateAddressType(value)
      );
    }
    let networkLu = [];
    if (value) {
  ...
  handleWalletSource = (value) => {
    if (this.state.cryptoData?.id === "00000000-0000-0000-0000-000000000000") {
      ...
      this.state.cryptoData?.id != "00000000-0000-0000-0000-000000000000" &&
      this.state.cryptoData.walletSource != "Others"
    ) {
      this.form?.current?.setFieldsValue({ otherWallet: null });
    }
  ...
  editDocuments = (docs) => {
    ...
    this.setState({
      ...this.state,
      details: { ...this.state.details, documents }
    });
  };
  handleDocCheck = (e) => {
  ...
  render() {
    ...
    if (showDeclartion) {
        ...
        <div className="custom-declaraton">
          ...
          <div className="success-pop text-center declaration-content">
            ...
            <Title level={2} className="success-title">
              Declaration form sent successfully
            </Title>
      ...
      return (
        <div className="custom-declaraton travel-success">
          {" "}
          <div className="success-pop text-center declaration-content">
            <Image
              src={success}
      ...
      return (
        <>
          ...
          >
            <Form.Item
              className="custom-forminput custom-label sc-error addcrypto-whitelist"
            ...
            <Row className="addcrypto-benficiary">
              ...
              {this.state.walletSourse === "Others" && (
                <Col xs={24} md={24} lg={24} xl={24} xxl={24}>
                  <Form.Item
                    className=" mb-8 px-4 text-white-50 custom-forminput custom-label pt-8 sc-error"
                    name="otherWallet"
              ...
              <Col xs={24} md={24} lg={24} xl={24} xxl={24}>
                ...
                >
                  <div
                    className={`d-flex  agree-check checkbox-mobile align-center`}
                  >
                    <label>
                      <input
                        type="checkbox"
                    ...
                    <div className="cust-agreecheck d-flex align-center">
                      I'm the owner of this wallet address{" "}
                      <span className="cust-start-style">*</span>
                    </div>
                  </div>
                </Form.Item>
              </Col>
              <Col xs={24} md={24} lg={24} xl={24} xxl={24}>
                ...
                >
                  ...
                  >
                    ...
                    <div className="cust-agreecheck d-flex align-center">
                      I may perform transactions greater than 15,000 CHF with
                      this address
                    </div>
                  </div>
                </Form.Item>
              ...
              <Col xs={24} md={24} lg={24} xl={24} xxl={24}>
                <Paragraph className="sub-abovesearch code-lbl upload-btn-mt">
                  Please upload a screenshot or video to prove you are the owner
                  of the address
                  {isDocCheck === true && (
                    <span className="cust-start-style">*</span>
                  )}
