...
const infoColumns = [
    ...
    { title: apiCalls.convertLocalLang('coin'), dataIndex: "symbol", render: (text, data) => <Link className="c-pointer" to={"/coindetails/" + data.id} onClick={() => store.dispatch(setSelctedCoinDetail(data))} ><span className="fs-18 fw-500 text-upper text-white mb-0 mt-12">{text.toUpperCase()}</span></Link>, sorter: (a, b) => ('' + a.symbol).localeCompare(b.symbol), },
    {
