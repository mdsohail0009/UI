...
const detailInfoColumns = [
    ...
    { title: "24h(%)", dataIndex: "price_change_percentage_24h", sorter: (a, b) => b.price_change_percentage_24h - a.price_change_percentage_24h, render: val => <><div className="coin-typo"> <div className={val < 0 ? 'text-red' : 'text-green'}>{val}<span>{val < 0 ? <span className="icon md lose ml-16" /> : <span className="icon md gain ml-12" />}</span></div></div></> },
