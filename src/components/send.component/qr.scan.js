...
class QRScan extends Component {
    ...
    get shareMenu() {
        return <Menu className="share-adrs">
            <Menu.Item>
                <WhatsappShareButton te url={process.env.REACT_APP_WEB_URL} title={`Hello, I would like to share my ${this.props?.sendReceive?.depositWallet?.walletCode} address for receive ${this.walletAddress}.Note:(Please make sure you are using the correct protocol otherwise you are risking of loosing the funds).I am using Suissebase. Thank you.`} >
                    <WhatsappIcon size={32} round={true} />
                </WhatsappShareButton>
            </Menu.Item>
            <Menu.Item>
                <EmailShareButton url={process.env.REACT_APP_WEB_URL} subject={"Wallet Address"} body={`Hello, I would like to share my ${this.props?.sendReceive?.depositWallet?.walletCode} address for receive ${this.walletAddress}.Note:(Please make sure you are using the correct protocol otherwise you are risking of loosing the funds).I am using Suissebase. Thank you.`}  >
