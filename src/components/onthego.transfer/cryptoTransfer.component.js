...
class OnthegoCryptoTransfer extends Component {
  ...
  componentDidMount() {
    this.enteramtForm?.current?.setFieldsValue({
      amount: this.props.sendReceive?.cryptoWithdraw?.selectedWallet
        ?.withdrawMinValue,
    })
  ...
  getPayees = async () => {
    ...
    let response = await apicalls.getPayeeCryptoLu(
      this.props.userProfile.id,
      this.props?.sendReceive?.cryptoWithdraw?.selectedWallet?.coin,
    )
    if (response.ok) {
      this.setState({
        ...this.state,
    ...
    let res = await apicalls.getPayeeCrypto(
      this.props.userProfile.id,
      this.props?.sendReceive?.cryptoWithdraw?.selectedWallet?.coin,
    )
    if (res.ok) {
  ...
  handlePreview = (item) => {
    ...
    this.props.dispatch(
      setWithdrawcrypto({
        ...obj,
        toWalletAddress: item.walletAddress,
  ...
  closeBuyDrawer = (obj) => {
    let showCrypto = false,
      showFiat = false
    if (obj) {
      if (obj.isCrypto) showCrypto = !obj?.close
      else showFiat = !obj?.close
    }
  ...
  amountNext = async (values) => {
    ...
    const {
      withdrawMaxValue,
      withdrawMinValue,
    } = this.props.sendReceive?.cryptoWithdraw?.selectedWallet
    this.setState({ ...this.state, error: null })
    if (amt === '') {
      this.setState({
        ...this.state,
      ...
      this.setState({
        ...this.state,
        errorMsg: null,
        error: apicalls.convertLocalLang('amount_min') + ' ' + withdrawMinValue,
      })
      this.myRef.current.scrollIntoView()
    }
    ...
    // }
      ...
      amt > this.props.sendReceive?.cryptoWithdraw?.selectedWallet?.coinBalance
    ) {
      this.setState({
        ...this.state,
        errorMsg: null,
      ...
      this.setState({ ...this.state, amount: amt }, () =>
        this.validateAmt(amt, 'newtransfer', values, 'newtransferLoader'),
      )
    }
  }
  ...
  validateAmt = async (amt, type, values, loader) => {
    ...
    this.props.dispatch(
      setSubTitle(apicalls.convertLocalLang('wallet_address')),
    )
    const validObj = {
      CustomerId: this.props.userProfile?.id,
      amount: amt ? amt : null,
    ...
    if (res.ok) {
      ...
      type == 'addressSelection'
        ? this.setState(
            { ...this.state, loading: false, [loader]: false, errorMsg: null },
            () => this.props.chnageStep(type, values),
          )
        : this.setState(
            {
              ...this.state,
      ...
      this.setState({
        ...
        [loader]: false,
        error: null,
        errorMsg: this.isErrorDispaly(res),
      })
      this.myRef.current.scrollIntoView()
    }
  }
  ...
  clickMinamnt(type) {
    ...
    if (type === 'all') {
      ...
      this.setState({
        ...
        amountPercentageType: 'all',
      })
      this.enteramtForm?.current?.setFieldsValue({ amount: obj.coinBalance })
    } else {
      this.setState({
        ...this.state,
        CryptoAmnt: this.props.sendReceive?.cryptoWithdraw?.selectedWallet
          ?.withdrawMinValue,
  ...
  renderStep = (step) => {
    ...
    const steps = {
            ...
            >
              ...
              {this.state.error != null && (
                <Alert
                  ...
                  onClose={() => this.setState({ ...this.state, error: null })}
                  showIcon
                />
              )}
              {this.state.errorMsg && (
              ...
              <Row
                gutter={[16, 16]}
                className="align-center send-crypto-err mx-4"
              >
                ...
                >
                  <div class="text-right mr-16">
                    <button
                      type="button"
                      class="ant-btn ant-btn-text ant-btn-sm min-btn "
                      onClick={() => this.clickMinamnt('min')}
                    >
                      <span>Min</span>
                    </button>
                    <button
      ...
      newtransfer: (
        <>
          <AddressCrypto
            onCancel={(obj) => this.closeBuyDrawer(obj)}
            cryptoTab={1}
            isShowheading={true}
          />
        </>
        ...
        <React.Fragment>
          <>
            ...
            <div className="mb-16" style={{ textAlign: 'center' }}>
              <text
                Paragraph
                className="fs-24 fw-600 text-white mb-16 mt-4 text-captz"
              >
                Who are you sending crypto to?
              </text>
            </div>
                      ...
                      <>
                        {
                          <Row
                            ...
                            onClick={async () => {
                              if (
                                ...
                                const res = await apicalls.confirmCryptoTransaction(
                                  {
                                    payeeId: item.id,
                                    reasonOfTransfer: '',
                          ...
                          >
                            ...
                            >
                              <label className="fs-16 fw-600 text-white l-height-normal c-pointer">
                                {item?.name} (
                                {item.walletAddress?.length > 0
                                  ? item.walletAddress.substring(0, 4) +
                                    `......` +
                ...
                >
                  {pastPayees.length > 0 &&
                    pastPayees?.map((item, idx) => (
                      ...
                      >
                        <Col xs={2} md={2} lg={2} xl={3} xxl={3} className="">
                          <div class="fund-circle text-white">
                            {item?.name?.charAt(0).toUpperCase()}
                          </div>
                        </Col>
...
)(withRouter(OnthegoCryptoTransfer))
