    ...
    const onSubmit = async (values) => {
        ...
        if (payeesave.ok) {
            ...
            if (props.type !== "manual") {
                const confirmRes = await confirmTransaction({ payeeId: payeesave.data.id, amount: props.onTheGoObj.amount, reasonOfTransfer: obj.reasonOfTransfer, documents: documents, ukShortCode: obj.payeeAccountModels[0]?.ukSortCode })
                if (confirmRes.ok) {
                    setBtnLoading(false);
                        ...
                        <>
                            <Row >
                                ...
                                {(props.currency != "CHF" && props.currency != "GBP") && <><Col xs={24} md={24} lg={24} xl={24} xxl={24}>
                                    ...
                                    <AddressDocumnet documents={documents || null} editDocument={edit} 
