...
class OnthegoFundTransfer extends Component {
  ...
  state = {
    ...
    addressOptions: {
      addressType: 'myself',
      transferType: this.props.selectedCurrency === 'EUR' ? 'sepa' : 'domestic',
    },
    isNewTransfer: false,
    amount: '',
  ...
  getPayees() {
    fetchPayees(this.props.userProfile.id, this.state.selectedCurrency).then(
      (response) => {
        if (response.ok) {
          this.setState({
            ...this.state,
  ...
  verificationCheck = async () => {
    ...
    if (verfResponse.ok) {
      for (let verifMethod in verfResponse.data) {
        if (
          [
            ...
            'twoFactorEnabled',
            'isLiveVerification',
          ].includes(verifMethod) &&
          verfResponse.data[verifMethod] === true
        ) {
      ...
      this.setState({
        ...this.state,
        isVarificationLoader: false,
        errorMessage: this.isErrorDispaly(verfResponse),
      })
    }
  }
  ...
  amountnext = (values) => {
    ...
    _amt = _amt.replace(/,/g, '')
    if (_amt > 0) {
      this.setState({ ...this.state, amount: _amt }, () =>
        this.validateAmt(_amt, 'newtransfer', values, 'newtransferLoader'),
      )
    } else {
      if (!_amt) {
  ...
  handleSearch = ({ target: { value: val } }) => {
    if (val) {
      const filterObj = this.state.payees.filter((item) =>
  ...
  saveWithdrawdata = async () => {
    ...
    if (this.state.verifyData?.verifyData) {
      ...
      this.setState({
        ...this.state,
        errorMessage: "Without Verifications you can't Proceed.",
      })
      this.reviewScrool.current.scrollIntoView()
      return
    }
    if (this.state.reviewDetails) {
      ...
      obj['accountNumber'] = obj.accountNumber
        ? apicalls.encryptValue(obj.accountNumber, this.props.userProfile?.sk)
        : null
      obj['bankName'] = obj.bankName
        ? apicalls.encryptValue(obj.bankName, this.props.userProfile?.sk)
        : null
      obj['bankAddress'] = obj.bankAddress
        ? apicalls.encryptValue(obj.bankAddress, this.props.userProfile?.sk)
        : null
      obj['beneficiaryAccountName'] = obj.beneficiaryAccountName
        ? apicalls.encryptValue(
            obj.beneficiaryAccountName,
            this.props.userProfile?.sk,
          )
        : null
      obj['beneficiaryAccountAddress'] = obj.beneficiaryAccountAddress
        ? apicalls.encryptValue(
            obj.beneficiaryAccountAddress,
            this.props.userProfile?.sk,
          )
        : null
      obj['routingNumber'] = obj.routingNumber
        ? apicalls.encryptValue(obj.routingNumber, this.props.userProfile?.sk)
        : null

      const saveRes = await saveWithdraw(obj)
      if (saveRes.ok) {
        this.chnageStep(
          this.state.isNewTransfer ? 'declaration' : 'successpage',
        )
  ...
  validateAmt = async (amt, step, values, loader) => {
    ...
    if (res.ok) {
      this.setState(
        { ...this.state, [loader]: false, errorMessage: null },
      ...
      this.setState({
        ...this.state,
        [loader]: false,
        errorMessage: this.isErrorDispaly(res),
      })
    }
  }
  ...
  renderStep = (step) => {
    ...
    const steps = {
      selectcurrency: (
        <React.Fragment>
          ...
          {!this.state.fiatWalletsLoading && (
            <div>
              <div className="mt-8">
                <Title className="sub-heading code-lbl">
                  Send from your Suissebase FIAT Wallet
                </Title>
              </div>
                ...
                locale={{
                  emptyText: (
                    <Empty
                      ...
                      description={<Translate content="No_data" />}
                    />
                  ),
                }}
                renderItem={(item) => (
                  <List.Item
                    onClick={() =>
                      this.setState(
                        { ...this.state, selectedCurrency: item.walletCode },
                        () => {
                          this.getPayees()
                  ...
                  >
                    <Link>
                      <List.Item.Meta
                        ...
                        title={
                          <div className="wallet-title">{item.walletCode}</div>
                        }
                      />
                      <>
                        <div className="text-right coin-typo">
                          <NumberFormat
                            value={item.amount}
                            className="text-white-30 fw-600"
                            displayType={'text'}
                            thousandSeparator={true}
                            prefix={item.walletCode == 'USD' ? '$' : '€'}
            ...
            >
              {!isVerificationEnable && (
                <Alert
                  ...
                  description={
                    <Text>
                      Without verifications you can't send. Please select send
                      verifications from{' '}
                      <a
                        onClick={() => {
                      ...
                      >
                        <NumberFormat
                          ...
                          onValueChange={() => {
                            this.setState({
                              ...this.state,
                              amount: this.enteramtForm.current?.getFieldsValue()
                                .amount,
                              errorMessage: '',
                            })
                          }}
      ...
      addressselection: (
        <React.Fragment>
          ...
          {!this.state.loading && (
            <>
              <Title
                className="fw-600 text-white px-4 mb-16 mt-16 text-captz"
                style={{ fontSize: '18px' }}
              >
                Address Book
              </Title>
              ...
              >
                    ...
                    <>
                      {
                        ...
                        >
                          <Col xs={6} md={2} lg={2} xl={3} xxl={3} className="">
                            <div class="fund-circle text-white">
                          ...
                          <Col
                            ...
                            xl={19}
                            xxl={19}
                            className="small-text-align"
                          >
                            <label className="fs-16 fw-600 text-white l-height-normal text-captz c-pointer">
                              {item.name}
                            </label>
                            {item.accountNumber && (
                              <div>
                                <Text className="fs-14 text-white-30 m-0">
                                  {this.state.selectedCurrency} account ending
                                  with{' '}
                                  {item.accountNumber?.substr(
                ...
                {!filterObj.length > 0 && (
                  <div
                    className="success-pop text-center"
                  ...
                  >
                    <img
                      ...
                      alt="Confirm"
                    />
                    <h1 className="fs-36 text-white-30 fw-200 mb-0">
                      {' '}
                      {apicalls.convertLocalLang('oops')}
                    </h1>
                    <p className="fs-16 text-white-30 fw-200 mb-0">
                      {' '}
                      {apicalls.convertLocalLang('address_available')}{' '}
                    </p>
                    <a onClick={() => this.chnageStep('newtransfer')}>
                      Click here to make new transfer
                    </a>
                  </div>
                )}
              </ul>
                  ...
                  pastPayees?.map((item, idx) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          <Text className="fs-14 text-white-30 m-0">
                            {this.state.selectedCurrency} account ending with{' '}
                            {item.accountNumber?.substr(
                              item.accountNumber?.length - 4,
                            )}
                          </Text>
      ...
      reasonfortransfer: (
        <React.Fragment>
          <div className="mb-16" style={{ textAlign: 'center' }}>
            <text
              Paragraph
              className="text-white fs-30 fw-600 px-4 mb-16 mt-4"
              style={{ fontSize: '18px ' }}
            >
              Transfer Details
            </text>
            ...
            <React.Fragment>
              ...
              <AddressDocumnet
                ...
                onDocumentsChange={(docs) => {
                  ...
                  this.setState({
                    ...this.state,
                    codeDetails: { ...this.state.codeDetails, documents },
                  })
                }}
                title={
                  'Please upload supporting docs to explain relationship with beneficiary*'
                }
              />
            </React.Fragment>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (
                        ...
                        validateFileds = validateFileds.concat([
                          'reasionOfTransfer',
                          'files',
                        ])
                      }
                      this.reasonForm.current
                        .validateFields(validateFileds)
                        .then(async () => {
      ...
      reviewdetails: (
        <React.Fragment>
          ...
          <div className="mb-16 text-center">
            <text Paragraph className="fs-24 fw-600 text-white mb-16 mt-4">
            ...
            >
              ...
              <Row gutter={24}>
                <Col xs={24} sm={24} md={24} lg={24} xxl={24}>
                  ...
                  >
                    <Text
                      className="fw-600 text-white mt-4 text-captz"
                      style={{ fontSize: '18px' }}
                    >
                      Transfer details
                    </Text>
                ...
                <Col xs={24} sm={24} md={24} lg={24} xxl={24}>
                  <div
                    ...
                    style={{ alignItems: 'baseline' }}
                  >
                    <Title className="fs-14 text-white fw-500 text-captz">
                      How much you will receive
                    </Title>
                    <Title className="fs-14 text-white fw-500 text-upper text-right">
                      <NumberFormat
                        value={`${
                          this.state.reviewDetails?.requestedAmount -
                          this.state.reviewDetails?.comission
                        }`}
                        thousandSeparator={true}
                        displayType={'text'}
                        decimalPlaces={2}
                      />{' '}
                      {`${this.state.reviewDetails?.walletCode}`}
                    </Title>
                  </div>
                </Col>
                <Col xs={24} sm={24} md={24} lg={24} xxl={24}>
              ...
              <Row gutter={24} className=" text-white mt-36">
                <Col xs={24} sm={24} md={24} lg={24} xxl={24}>
                  ...
                  >
                    <Text
                      className="fw-600 text-white mb-0 mt-4 text-captz"
                      style={{ fontSize: '18px' }}
                    >
                      Recipient details
                    </Text>
                ...
                <Col xs={24} sm={24} md={24} lg={24} xxl={24}>
                  ...
                  >
                    ...
                    <Title className="fs-14 fw-500 text-white text-right plist-textwrap">
                      {this.state.reviewDetails?.favouriteName}
                    </Title>
                  </div>
                </Col>
                ...
                <Col xs={24} sm={24} md={24} lg={24} xxl={24}>
                  <Verifications
                    onchangeData={(obj) => this.changesVerification(obj)}
                    onReviewDetailsLoading={(val) =>
                      this.onReviewDetailsLoading(val)
                    }
                  />
                </Col>
                    ...
                    <div className="text-right mt-36 create-account">
                      <Form.Item className="mb-0 mt-16">
                        ...
                        <Button
                          ...
                          style={{
                            backgroundColor: !isShowGreyButton && '#ccc',
                            borderColor: !isShowGreyButton && '#3d3d3d',
                          }}
      ...
      newtransfer: (
        <>
          <FiatAddress
            typeOntheGo={this.props?.ontheGoType}
            ...
            onTheGoObj={this.state.onTheGoObj}
          />
        </>
      ),
      declaration: (
        <div className="custom-declaraton">
          {' '}
          <div className="text-center mt-36 declaration-content">
...
)(withRouter(OnthegoFundTransfer))
