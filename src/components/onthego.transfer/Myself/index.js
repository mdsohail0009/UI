...
const MyselfNewTransfer = ({ currency, isBusiness, onTheGoObj, ...props }) => {
  ...
  const [addressOptions, setAddressOptions] = useState({
    addressType: "myself",
    transferType: currency === "EUR" ? "sepa" : "swift",
  ...
  const getRecipientDetails = async () => {
    setLoader(true);
    const response = await apiCalls.getRecipientData(
      props.userConfig.id,
  ...
  const saveTransfer = async (values) => {
    ...
    saveObj.payeeAccountModels[0].accountNumber =
      currency == "USD" ? values.accountNumber : null;
    ...
    saveObj.payeeAccountModels[0].line1 =
      currency == "USD" ? values.line1 : null;
    saveObj.payeeAccountModels[0].line2 =
      currency == "USD" ? values.line2 : null;
    saveObj.payeeAccountModels[0].state = bankDetails.state
      ? bankDetails.state
      : null;
    saveObj.payeeAccountModels[0].bankName =
    ...
    saveObj.payeeAccountModels[0].branch = bankDetails.branch
      ? bankDetails.branch
      : null;
    saveObj.payeeAccountModels[0].country = bankDetails.country
      ? bankDetails.country
      : null;
    ...
    if (response.ok) {
      if (props.type !== "manual") {
        const confirmRes = await confirmTransaction({
          payeeId: response.data.id,
    ...
    if (e?.length >= 10 && isValid) {
      ...
      if (response.ok) {
        if (
          isValid &&
          response.data &&
          (response.data?.routingNumber || response.data?.bankName)
        ) {
          setValidIban(true);
          ...
          if (
            ...
            setbankDetails({});
            seterrorMessage(
              "No bank details are available for this IBAN number"
            );
            useDivRef.current.scrollIntoView();
            return;
          }
            ...
            <>
                ...
                <>
                  <Row gutter={[16, 16]}>
                    <Col xs={24} md={24} lg={24} xl={24} xxl={24} className="">
                      <Tabs
                        style={{ color: "#fff" }}
                        className="cust-tabs-fait"
                        onChange={(activekey) => {
                          setAddressOptions({
                            ...addressOptions,
                            domesticType: activekey,
                            tabType: activekey
                          });
                          form.resetFields();
                          seterrorMessage(null);
                        ...
                        activeKey={addressOptions.tabType}
                      >
                        <Tabs.TabPane
              ...
              {!isLoading && (
                <>
                  ...
                  <div className="box basic-info alert-info-custom kpi-List">
                    <Row>
                      ...
                      >
                        <div>
                          {" "}
                          <label className="kpi-label">Address Line 1</label>
                          <div>
                            <Text className="kpi-val">
                  ...
                  <h2
                    style={{ fontSize: 14 }}
                    className="mt-36 text-captz px-4 text-white fw-600"
                  >
                    Bank Details
                  </h2>
                  {(currency == "EUR" ||
                        ...
                        {validIban && isShowBankDetails && (
                          <Row>
                            <Col
                              ...
                              className="mb-16"
                            >
                              <label className="fs-12 fw-500">Bank Name</label>
                              <div className="pr-24">
                                <Text className="fs-14 fw-400 text-white">
                                  {bankDetails?.bankName != "" &&
