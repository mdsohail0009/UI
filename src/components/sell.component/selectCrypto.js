...
class SelectSellCrypto extends Component {
    ...
    previewSellData() {
        ...
        if ((!this.state.USDAmnt && !this.state.CryptoAmnt) || (parseFloat(this.state.USDAmnt) === 0 || parseFloat(this.state.CryptoAmnt) === 0)) {
            this.setState({ ...this.state, errorMessage: apicalls.convertLocalLang('enter_amount')
        })
            this.myRef.current.scrollIntoView();
        }
        else if (!obj.toWalletId) {
            this.setState({ ...this.state, errorMessage: apicalls.convertLocalLang('pleaseSelectWallet')
        })
            this.myRef.current.scrollIntoView();
            return;
