                ...
                {this.props.sellData.MemberCoins&&<div className="sellcrypto-container auto-scroll">
                    {this.props.sellData.MemberCoins.map((coin,idx) => <Card className="crypto-card select mb-16 c-pointer" bordered={false} onClick={() => {this.props.changeStep('step10');this.props.dispatch(updateCoinDetails(coin))}} >
                        <span className="d-flex align-center">
                            ...
                            <Text className="fs-24 text-white crypto-name ml-8">{coin.coin}</Text>
                        </span>
                        <div className="crypto-details">
                            <Text className="crypto-percent text-white fw-700">{coin.change}<sup className="percent text-white fw-700">%</sup></Text>
                            <div className="fs-16 text-white-30 fw-200 crypto-amount">
                                <div className="text-yellow">{coin.balance}<Text className="text-secondary"> {coin.coin}</Text></div>
                                <div className="text-yellow"><Text className="text-secondary">$</Text> {coin.price}</div>
                            </div>
                        </div>
                    </Card>)}
                </div>}
